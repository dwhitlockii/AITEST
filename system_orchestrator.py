...
class SystemOrchestrator:
    ...
    async def _initialize_agents(self):
        ...
        try:
            ...
            self.agents["predictive"] = PredictiveAgent()
            self.agents["compliance"] = ComplianceAgent()
            self.agents["backup"] = BackupAgent()
        ...
        for name, agent in self.agents.items():
            try:
                if not agent.running:
                    ...
                    system_logger.info(
                        f"Agent '{name}' started successfully (task launched)."
                    )
                else:
                    system_logger.warning(
                        f"Agent '{name}' was already running. Reset and started."
                    )
            except Exception as e:
    ...
    async def _restart_agent(self, agent_name: str):
        ...
        try:
            ...
            if agent_name == "sensor":
                ...
                self.agents[agent_name] = SecurityAgent()
            elif agent_name == "network":
                self.agents[agent_name] = NetworkAgent()
            elif agent_name == "application":
                self.agents[agent_name] = ApplicationAgent()
            elif agent_name == "predictive":
                self.agents[agent_name] = PredictiveAgent()
    ...
    async def _log_system_status(self):
        ...
        try:
            ...
            status_data = {
                ...
                "total_agents": len(self.agents),
                "running_agents": sum(
                    1 for agent in self.agents.values() if agent.running
                ),
                "agent_health": {
                    name: agent.health_status for name, agent in self.agents.items()
                },
            }
                ...
                try:
                    ...
                    if agent_health["status"] == "critical":
                        ...
                        health_data["issues"].append(
                            f"Agent {agent_name} is in critical condition"
                        )
                    elif (
                        agent_health["status"] == "warning"
                        and health_data["overall_status"] == "healthy"
                    ):
                        health_data["overall_status"] = "warning"
                        health_data["issues"].append(
                            f"Agent {agent_name} is showing warning signs"
                        )
    ...
    def get_system_info(self) -> Dict[str, Any]:
        ...
        try:
            total_agents = len(self.agents)
            running_agents = sum(
                1 for agent in self.agents.values() if getattr(agent, "running", False)
            )
            uptime = (
                (datetime.now() - self.startup_time).total_seconds()
                if self.startup_time
            ...
            for name, agent in self.agents.items():
                ...
                agent_details[name] = {
                    "type": agent_type,
                    "status": (
                        "running" if getattr(agent, "running", False) else "stopped"
                    ),
                    "health": getattr(agent, "health_status", "unknown"),
                    "uptime": (
                        (datetime.now() - agent.start_time).total_seconds()
                        if getattr(agent, "start_time", None)
                        else 0
                    ),
