...
class PerformanceMonitor:
    ...
    def add_check(
        self, name: str, result: bool, details: str, recommendation: str = None
    ):
    ...
    async def check_python_environment(self):
        ...
        # Required packages
        required_packages = ["fastapi", "uvicorn", "psutil", "asyncio", "sqlite3"]
    ...
    async def check_agent_system(self):
        ...
        try:
            ...
            agent_count = (
                len(orchestrator.agents) if hasattr(orchestrator, "agents") else 0
            )
            self.add_check(
                "Agent System",
                agent_count > 0,
            ...
            try:
                ...
                self.add_check("Message Bus", True, "Message bus is available", None)
            except Exception as e:
                self.add_check(
                    "Message Bus",
                    False,
        ...
        try:
            ...
            self.add_check(
                ...
                (
                    ...
                    if is_healthy
                    else "Ollama is not responding"
                ),
                (
                    "Start Ollama service or check configuration"
                    if not is_healthy
                    else None
                ),
            )
