...
class NetworkAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        # Network monitoring
        self.target_hosts = ["8.8.8.8", "1.1.1.1", "google.com"]  # DNS servers and common sites
    ...
    async def _perform_check(self):
        ...
        try:
            ...
            await self._update_network_status(
                network_metrics, connectivity_results, bandwidth_results, performance_analysis, issues
            )
            ...
            for host in self.target_hosts:
                try:
                    ...
                    connectivity_results[host] = {
                        "ping": ping_result,
                        "ports": port_results,
                        "overall_status": "reachable" if ping_result.get("success") else "unreachable"
                    }
            ...
            if result.returncode == 0:
                ...
                for line in output_lines:
                    if "time=" in line or "time<" in line:
                        try:
                            # Extract time value
                            time_part = line.split("time=")[1].split()[0] if "time=" in line else "1"
    ...
    def _calculate_network_score(
        ...
        try:
            ...
            if network_errors.get("errors_in", 0) > 0 or network_errors.get("errors_out", 0) > 0:
                score -= 15
            
            # Check DNS resolution
            dns_status = metrics.get("dns_status", {})
            dns_failures = sum(1 for result in dns_status.values() if not result.get("resolved", True))
            if dns_failures > 0:
    ...
    async def _perform_network_remediation(self, issues: List[Dict[str, Any]]):
        ...
        try:
            for issue in issues:
                ...
                self.logger.warning(f"Network remediation performed for {issue_type}: {issue.get('description')}")
    ...
    async def _update_network_status(
        self, metrics: Dict[str, Any], connectivity: Dict[str, Any], 
        bandwidth: Dict[str, Any], analysis: Dict[str, Any], issues: List[Dict[str, Any]]
    ):
        """Update network status."""
        try:
    ...
    async def _handle_network_alert(self, message):
        ...
        if "network" in alert_content.get("type", "").lower():
            self.logger.warning(f"Network alert received: {alert_content.get('message', 'Unknown alert')}")
    ...
    def get_network_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "network_score": self._calculate_network_score(
                ...
                self.network_metrics[-1]["connectivity"] if self.network_metrics else {},
