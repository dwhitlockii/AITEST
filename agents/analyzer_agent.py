...
class AnalyzerAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        # Select LLM provider
        self.llm_provider = config.agent_ai_provider.get("analyzer", "openai")
    ...
    async def _analyze_system_health(self, metrics: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Use LLM to analyze system health."""
        if not metrics:
        ...
        try:
            ...
            # Ensure gpt_decision is a GPTDecision or dict with expected keys
            if hasattr(gpt_decision, 'decision') and hasattr(gpt_decision, 'reasoning'):
                ...
                self.logger.warning("LLM returned a dict instead of GPTDecision. Using dict keys.")
                ...
                self.logger.error(f"LLM returned unexpected type: {type(gpt_decision)}")
                return {
                    "timestamp": datetime.now().isoformat(),
                    "error": f"LLM returned unexpected type: {type(gpt_decision)}",
                    "fallback_analysis": self._fallback_analysis(latest_metrics)
                }
        except LLMQuotaExceeded:
            if config.fallback_mode_on_llm_quota_exceeded:
                self.logger.warning("OpenAI quota exceeded. Using rule-based fallback analysis.")
                return {
                    "timestamp": datetime.now().isoformat(),
                    "error": "LLM quota exceeded. Using rule-based fallback.",
                    "fallback_analysis": self._fallback_analysis(latest_metrics)
                }
            else:
                self.logger.warning("OpenAI quota exceeded. Trying Gemini fallback.")
                try:
                    gpt_decision = await gemini_client.analyze_metrics(latest_metrics, context)
                    if hasattr(gpt_decision, 'decision') and hasattr(gpt_decision, 'reasoning'):
                        self.log_gpt_decision(gpt_decision.decision, gpt_decision.reasoning)
                        analysis_result = {
                            "timestamp": datetime.now().isoformat(),
                        ...
                        self.logger.warning("Gemini returned a dict instead of GPTDecision. Using dict keys.")
                        self.log_gpt_decision(gpt_decision.get("decision", "?"), gpt_decision.get("reasoning", "?"))
                        analysis_result = {
                            "timestamp": datetime.now().isoformat(),
                            "gpt_analysis": gpt_decision,
                        ...
                        self.logger.error(f"Gemini returned unexpected type: {type(gpt_decision)}")
                        return {
                            "timestamp": datetime.now().isoformat(),
                            "error": f"Gemini returned unexpected type: {type(gpt_decision)}",
                            "fallback_analysis": self._fallback_analysis(latest_metrics)
                        }
                except Exception as e:
    ...
    def _build_analysis_context(self, metrics: List[Dict[str, Any]]) -> str:
        ...
        if recent_cpu:
            ...
            cpu_trend = "increasing" if recent_cpu[-1] > recent_cpu[0] else "decreasing" if recent_cpu[-1] < recent_cpu[0] else "stable"
            context_parts.append(f"CPU usage: {avg_cpu:.1f}% average, {cpu_trend} trend")
        
        if recent_memory:
            avg_memory = sum(recent_memory) / len(recent_memory)
            memory_trend = "increasing" if recent_memory[-1] > recent_memory[0] else "decreasing" if recent_memory[-1] < recent_memory[0] else "stable"
            context_parts.append(f"Memory usage: {avg_memory:.1f}% average, {memory_trend} trend")
        ...
        if "health_status" in latest_performance:
            context_parts.append(f"Overall health: {latest_performance['health_status']}")
        
        if "bottlenecks" in latest_performance and latest_performance["bottlenecks"]:
            context_parts.append(f"Bottlenecks: {', '.join(latest_performance['bottlenecks'])}")
        
        return " | ".join(context_parts) if context_parts else "Standard system monitoring"
    ...
    def _extract_issues_from_gpt(self, gpt_decision) -> List[Dict[str, Any]]:
        ...
        for alternative in gpt_decision.alternatives:
            if any(keyword in alternative.lower() for keyword in ["restart", "kill", "stop", "clean", "free"]):
                issues.append({
    ...
    def _calculate_health_score(self, metrics: Dict[str, Any]) -> float:
        ...
        try:
            ...
            for service_name, service_data in metrics.get("services", {}).items():
                if isinstance(service_data, dict) and not service_data.get("running", True):
                    score -= 5
    ...
    async def _check_threshold_violations(self, metrics: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def _analyze_trends(self, metrics: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            # Use GPT for trend prediction
            gpt_decision = await gpt_client.detect_anomalies(metrics, self.metric_history[-10:])
    ...
    async def _generate_alerts(self, violations: List[Dict[str, Any]], analysis_result: Dict[str, Any]):
        ...
        for alert in alerts:
            await self.message_bus.broadcast(
                ...
                content=alert,
                priority=MessagePriority.HIGH if alert["severity"] in ["critical", "high"] else MessagePriority.NORMAL
            )
            
            self.logger.warning(f"Alert generated: {alert['message']}")
    
    async def _broadcast_analysis_results(self, analysis_result: Dict[str, Any], 
    ...
    def get_analysis_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "gpt_confidence": latest_analysis.get("gpt_analysis", {}).get("confidence", 0),
            "risk_level": latest_analysis.get("gpt_analysis", {}).get("risk_level", "unknown")
        }

    async def _perform_gpt_analysis(self, metrics: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            gpt_decision = await gpt_client.analyze_metrics(metrics, context)
    ...
    async def _perform_trend_analysis(self, metrics: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Perform trend analysis using GPT."""
        try:
            # Get historical context for trend analysis
            historical_context = self.metric_history[-10:] if len(self.metric_history) >= 10 else self.metric_history
            
            # Use the new GPT client method for anomaly detection
            gpt_decision = await gpt_client.detect_anomalies(metrics, historical_context)
