...
class CommunicatorAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        self.logger.info("CommunicatorAgent initialized - ready to keep everyone in the loop")
    ...
    async def _generate_system_summary(self):
        ...
        if (self.last_system_summary and 
            (current_time - self.last_system_summary).seconds < self.summary_interval):
            return
    ...
    def _log_system_summary(self, summary: Dict[str, Any]):
        ...
        try:
            ...
            if overall_health == "healthy":
                self.logger.success("System is running smoothly - all systems operational")
            elif overall_health == "warning":
                self.logger.warning("System showing some stress - keeping an eye on things")
            elif overall_health == "critical":
                self.logger.critical("System is in critical condition - immediate attention required")
            ...
            self.logger.info(f"Agent status: {running_agents}/{total_agents} agents running")
            
            # Log active issues
            if issues:
                self.logger.warning(f"Active issues: {len(issues)} issues requiring attention")
    ...
    def _cleanup_old_data(self):
        ...
        self.system_health_history = [
            summary for summary in self.system_health_history
            if datetime.fromisoformat(summary.get("timestamp", "1970-01-01")) > cutoff_time
        ]
    ...
    async def _handle_message(self, message):
        ...
        if message.priority in [MessagePriority.HIGH, MessagePriority.CRITICAL, MessagePriority.EMERGENCY]:
            self.logger.warning(f"High priority message from {message.sender}: {message.type.value}")
        ...
        if message.type == MessageType.ALERT:
            ...
            await self.message_bus.broadcast(
                ...
                content={"info": f"{self.agent_name} received system command: {message.content.get('command', 'unknown')}"},
                priority=MessagePriority.NORMAL
            )
    ...
    def get_communication_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_issues": len(self.issues_detected),
            "recent_messages": [
                {"timestamp": msg.get("timestamp", "unknown"), "description": msg.get("description") or f"Message from {msg.get('sender', 'unknown')}: {msg.get('type', 'unknown')}"}
                for msg in self.message_history[-10:]
            ] if self.message_history else [],
            "system_health_file": self.status_file
        }
    
    def add_user_notification(self, message: str, notification_type: str = "info", severity: str = "medium"):
