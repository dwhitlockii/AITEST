...
from utils.message_bus import MessageBus, Message, MessageType, MessagePriority, message_bus
...
class BaseAgent(ABC):
    ...
    async def _setup_subscriptions(self):
        ...
        await self.message_bus.subscribe(MessageType.STATUS_UPDATE, self._handle_status_update)
        self.subscribed_message_types.append(MessageType.STATUS_UPDATE)
        
        # Subscribe to system commands
        await self.message_bus.subscribe(MessageType.SYSTEM_COMMAND, self._handle_system_command)
    ...
    async def _cleanup_subscriptions(self):
        ...
        for message_type in self.subscribed_message_types:
            try:
                await self.message_bus.unsubscribe(message_type, self._handle_status_update)
                await self.message_bus.unsubscribe(message_type, self._handle_system_command)
            except Exception as e:
    ...
    async def _handle_status_update(self, message: Message):
        ...
        self.logger.debug(f"Received status update from {message.sender}: {message.content}")
    ...
    def log_metric(self, metric_name: str, value: Any, threshold: Optional[float] = None):
