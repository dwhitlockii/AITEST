...
class CLIInterface:
    ...
    async def _show_system_status(self):
        ...
        try:
            ...
            # Create status panel
            status_text = """
[bold]System Health:[/bold] {system_health}
[bold]Uptime:[/bold] {uptime:.0f} seconds
[bold]Agents:[/bold] {running_agents}/{total_agents} running
[bold]Start Time:[/bold] {startup_time}
            """

            health_color = (
                "green"
                if system_info.get("system_health") == "healthy"
                else (
                    "yellow"
                    if system_info.get("system_health") == "warning"
                    else (
                        "red"
                        if system_info.get("system_health") == "critical"
                        else "white"
                    )
                )
            )
    ...
    async def _show_detailed_status(self):
        ...
        try:
            ...
            # System overview
            console.print(
                f"[bold]System Health:[/bold] {system_info.get('system_health', 'Unknown')}"
            )
            console.print(
                f"[bold]Uptime:[/bold] {system_info.get('uptime', 0):.0f} seconds"
            )
            console.print(
                f"[bold]Total Agents:[/bold] {system_info.get('total_agents', 0)}"
            )
            console.print(
                f"[bold]Running Agents:[/bold] {system_info.get('running_agents', 0)}"
            )
            ...
            for name, details in system_info.get("agent_details", {}).items():
                ...
                health_color = (
                    "green"
                    if details.get("health") == "healthy"
                    else (
                        "yellow"
                        if details.get("health") == "warning"
                        else "red" if details.get("health") == "critical" else "white"
                    )
                )
            ...
            if sensor_agent and hasattr(sensor_agent, "get_current_metrics"):
                ...
                if metrics:
                    ...
                    if "cpu" in metrics:
                        cpu = metrics["cpu"]
                        console.print(
                            f"[bold]CPU Usage:[/bold] {cpu.get('usage_percent', 'N/A')}%"
                        )
                        console.print(
                            f"[bold]CPU Cores:[/bold] {cpu.get('core_count', 'N/A')}"
                        )
                    ...
                    if "memory" in metrics:
                        ...
                        console.print(
                            f"[bold]Memory Usage:[/bold] {memory.get('usage_percent', 'N/A')}%"
                        )
                        console.print(
                            f"[bold]Memory Total:[/bold] {memory.get('total_gb', 'N/A'):.1f} GB"
                        )
                        console.print(
                            f"[bold]Memory Available:[/bold] {memory.get('available_gb', 'N/A'):.1f} GB"
                        )
                    ...
                    if "disk" in metrics:
                        ...
                        for path, disk in metrics["disk"].items():
                            console.print(
                                f"  {path}: {disk.get('usage_percent', 'N/A')}%"
                            )
                    ...
                    if "performance" in metrics:
                        ...
                        console.print(
                            f"[bold]Health Status:[/bold] {perf.get('health_status', 'N/A')}"
                        )
                        console.print(
                            f"[bold]Performance Trend:[/bold] {perf.get('performance_trend', 'N/A')}"
                        )
        ...
        try:
            ...
            if analyzer_agent and hasattr(analyzer_agent, "get_analysis_summary"):
                ...
                if "error" not in summary:
                    console.print(
                        f"[bold]Health Score:[/bold] {summary.get('health_score', 'N/A')}"
                    )
                    console.print(
                        f"[bold]Issues Detected:[/bold] {summary.get('issues_detected', 0)}"
                    )
                    console.print(
                        f"[bold]GPT Confidence:[/bold] {summary.get('gpt_confidence', 'N/A')}"
                    )
                    console.print(
                    ...
                    if summary.get("latest_analysis"):
                        ...
                        if "gpt_analysis" in analysis:
                            ...
                            console.print(
                                f"[bold]Reasoning:[/bold] {gpt.get('reasoning', 'N/A')}"
                            )
    ...
    async def _show_remediation_status(self):
        ...
        try:
            ...
            if remediator_agent and hasattr(
                remediator_agent, "get_remediation_summary"
            ):
                summary = remediator_agent.get_remediation_summary()

                console.print(
                    f"[bold]Total Actions:[/bold] {summary.get('total_actions', 0)}"
                )
                console.print(
                    f"[bold]Successful Remediations:[/bold] {summary.get('successful_remediations', 0)}"
                )
                console.print(
                    f"[bold]Failed Remediations:[/bold] {summary.get('failed_remediations', 0)}"
                )
                console.print(
                    f"[bold]Success Rate:[/bold] {summary.get('success_rate', 0):.1f}%"
                )
                console.print(
                    f"[bold]Active Remediations:[/bold] {summary.get('active_remediations', 0)}"
                )
                ...
                if summary.get("recent_attempts"):
                    ...
                    for attempt in summary["recent_attempts"][-5:]:
                        ...
                        console.print(
                            f"  [{success_color}]{'✓' if attempt.get('success') else '✗'}[/{success_color}] {attempt.get('action', 'Unknown')}"
                        )
        ...
        try:
            ...
            if recent_messages:
                ...
                for msg in recent_messages:
                    console.print(
                        f"  [{msg.get('timestamp', 'Unknown')}] {msg.get('sender', 'Unknown')} → {msg.get('type', 'Unknown')}"
                    )
    ...
    async def _show_configuration(self):
        ...
        try:
            ...
            for agent_name, agent_config in agents_config.items():
                console.print(
                    f"  {agent_name}: {agent_config.get('check_interval', 'N/A')}s interval"
                )
    ...
    async def _send_commands(self):
        ...
        try:
            ...
            commands = {"1": "status", "2": "health_check", "3": "restart"}

            if choice in commands:
                command = commands[choice]
                target = Prompt.ask(
                    "Target (all/sensor/analyzer/remediator/communicator)",
                    default="all",
                )

                console.print(
                    f"[yellow]Sending command '{command}' to {target}...[/yellow]"
                )
                ...
                console.print(
                    f"[yellow]Sending custom command '{command}' to {target}...[/yellow]"
                )
                await orchestrator.send_command(command, target)
                console.print("[green]Command sent successfully![/green]")
        ...
        try:
            with Live(auto_refresh=True, refresh_per_second=1) as live:
                while True:
                    ...
                    for name, details in system_info.get("agent_details", {}).items():
                        status_icon = (
                            "🟢" if details.get("status") == "running" else "🔴"
                        )
                    ...
                    try:
                        sensor_agent = orchestrator.agents.get("sensor")
                        if sensor_agent and hasattr(
                            sensor_agent, "get_current_metrics"
                        ):
                            metrics = sensor_agent.get_current_metrics()
                            metrics_text = ""
                            ...
                            layout["right"].update(
                                Panel(metrics_text, title="System Metrics")
                            )
                    except Exception as e:
                        layout["right"].update(
                            Panel(f"Metrics unavailable: {e}", title="System Metrics")
                        )
