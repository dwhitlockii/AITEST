...
class MessageType(Enum):
    ...
    # New message types for additional agents
...
class MessageBus:
    ...
    def __init__(self, max_queue_size: int = 1000, retention_time: int = 300):
        ...
        self.queues: Dict[MessagePriority, asyncio.Queue] = {
            priority: asyncio.Queue(
                maxsize=max_queue_size // 5
            )  # Divide by priority levels
    ...
    async def publish(self, message: Message) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Message published: {message.type.value} from {message.sender}"
            )
            return True

        except asyncio.QueueFull:
            logger.warning(
                f"Message queue full, dropping message: {message.type.value}"
            )
            return False
    ...
    async def get_message(
        self, priority: MessagePriority = MessagePriority.NORMAL, timeout: float = 1.0
    ) -> Optional[Message]:
                ...
                if p.value >= priority.value:
                    try:
                        return await asyncio.wait_for(
                            self.queues[p].get(), timeout=timeout
                        )
    ...
    async def send_direct(
        ...
        if isinstance(priority, str):
            priority = (
                MessagePriority[priority.upper()]
                if priority.isalpha()
                else MessagePriority(int(priority))
            )
        message = Message(
            id=str(uuid.uuid4()),
            type=message_type,
