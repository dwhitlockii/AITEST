...
class SystemLogger:
    ...
    def __init__(self, name: str = "AgentSystem", log_file: str = "debug_output/agent_system.log"):
        self.name = name
    ...
    def metric_alert(self, metric: str, value: float, threshold: float, **kwargs):
        """Log metric alert."""
        self.logger.warning(f"📊 METRIC ALERT: {metric} = {value} (threshold: {threshold})", **kwargs)
    
    def remediation_action(self, action: str, target: str, **kwargs):
        """Log remediation action."""
        self.logger.info(f"🔧 REMEDIATION: {action} on {target}", **kwargs)
    
    def metric(self, metric_name: str, value: Any, threshold: Optional[float] = None, **kwargs):
        """Log a metric with optional threshold comparison."""
        if threshold:
            status = "🟢" if value <= threshold else "🔴"
            self.logger.info(f"📊 {metric_name}: {value} (threshold: {threshold}) {status}", **kwargs)
        else:
    ...
    def gpt_decision(self, decision: str, reasoning: str, **kwargs):
        ...
        self.logger.info(f"🧠 GPT Decision: {decision} | Reasoning: {reasoning}", **kwargs)
    
    def health_check(self, status: str, details: Dict[str, Any], **kwargs):
        """Log system health status."""
        status_emoji = "🟢" if status == "healthy" else "🔴" if status == "critical" else "🟡"
...
class AgentLogger:
    ...
    def metric(self, metric_name: str, value: Any, threshold: Optional[float] = None, **kwargs):
        ...
        if threshold:
            status = "🟢" if value <= threshold else "🔴"
            self.logger.info(f"{emoji} {self.agent_name}: 📊 {metric_name}: {value} (threshold: {threshold}) {status}", **kwargs)
        else:
            self.logger.info(f"{emoji} {self.agent_name}: 📊 {metric_name}: {value}", **kwargs)
    
    def action(self, action: str, target: str, result: str = "initiated", **kwargs):
        """Log an agent action."""
        emoji = system_logger._get_agent_emoji(self.agent_name)
        self.logger.info(f"{emoji} {self.agent_name}: 🎯 {action} on {target} - {result}", **kwargs)
    
    def gpt_decision(self, decision: str, reasoning: str, **kwargs):
        """Log a GPT-based decision."""
        emoji = system_logger._get_agent_emoji(self.agent_name)
        self.logger.info(f"{emoji} {self.agent_name}: 🧠 GPT Decision: {decision} | Reasoning: {reasoning}", **kwargs)
