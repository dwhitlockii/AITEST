    ...
    async def _ensure_model(self):
        ...
        try:
            ...
            if self.model not in available_models:
                logger.warning(f"Configured model '{self.model}' not available. Falling back to '{self.fallback_model}'")
                self.model = self.fallback_model
        except Exception as e:
            logger.warning(f"Could not verify model availability: {e}")
        self._model_checked = True
    
    async def analyze_metrics(self, metrics: Dict[str, Any], context: str = "") -> GPTDecision:
        """
        ...
        try:
            ...
            logger.info(f"Analysis completed: {decision.decision} (confidence: {decision.confidence})")
            return decision
            
        except Exception as e:
            logger.error(f"Analysis failed: {e}")
            return self._create_fallback_decision("Continue monitoring", 
                                                f"Analysis failed: {e}", 0.5)
    
    async def recommend_remediation(self, issue: str, metrics: Dict[str, Any], 
                                  available_actions: List[str]) -> GPTDecision:
        """
        ...
        try:
            ...
            logger.info(f"Remediation recommended: {decision.decision} (risk: {decision.risk_level})")
            return decision
            
        except Exception as e:
            logger.error(f"Remediation recommendation failed: {e}")
            return self._create_fallback_decision("Monitor situation", 
                                                f"Recommendation failed: {e}", 0.3)
    
    async def detect_anomalies(self, metrics: Dict[str, Any], 
                             historical_context: List[Dict[str, Any]] = None) -> GPTDecision:
        """
        ...
        try:
            ...
            logger.info(f"Anomaly detection: {decision.decision} (confidence: {decision.confidence})")
            return decision
            
        except Exception as e:
            logger.error(f"Anomaly detection failed: {e}")
            return self._create_fallback_decision("No anomalies detected", 
                                                f"Detection failed: {e}", 0.4)
...
"""
    ...
    def _build_remediation_prompt(self, issue: str, metrics: Dict[str, Any], 
...
"""
    ...
    def _build_anomaly_prompt(self, metrics: Dict[str, Any], 
...
"""
    ...
    async def _make_request(self, prompt: str) -> str:
        ...
        for attempt in range(self.retry_attempts):
            try:
                logger.info(f"Making GPT request (attempt {attempt + 1}) with model {self.model}")
                response = await self.client.chat.completions.create(
                    model=self.model,
                    messages=[
                        {"role": "system", "content": "You are an intelligent system monitoring and remediation AI. Provide accurate, safe, and actionable recommendations."},
                ...
                if hasattr(e, 'status_code') and e.status_code == 404:
                    logger.warning(f"Model '{self.model}' not found (404). Falling back to '{self.fallback_model}' and retrying.")
                    self.model = self.fallback_model
                    self._model_checked = True
                    continue
                if 'model_not_found' in str(e):
                    logger.warning(f"Model '{self.model}' not found. Falling back to '{self.fallback_model}' and retrying.")
                    self.model = self.fallback_model
                    self._model_checked = True
                    continue
                # Check for quota exceeded (429 insufficient_quota)
                if (hasattr(e, 'status_code') and e.status_code == 429) or 'insufficient_quota' in str(e):
    ...
    def _parse_analysis_response(self, response: str) -> GPTDecision:
        ...
        try:
            ...
            return self._create_fallback_decision("Analysis parsing failed", str(e), 0.3)
    ...
    def _parse_remediation_response(self, response: str) -> GPTDecision:
        ...
        try:
            ...
            return self._create_fallback_decision("Remediation parsing failed", str(e), 0.3)
    ...
    def _apply_safety_checks(self, decision: GPTDecision, metrics: Dict[str, Any]) -> GPTDecision:
        ...
        if cpu_high or memory_high:
            logger.warning(f"System under stress, downgrading risk level from {decision.risk_level}")
    ...
    def _create_fallback_decision(self, decision: str, reasoning: str, confidence: float) -> GPTDecision:
